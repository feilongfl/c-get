#!/usr/bin/env sh
echo build linux ...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o c-get-linux-amd64 -ldflags "-X main._version_='"`git describe --abbrev=0 --tags`"' -X main._commit_='"`git log --pretty=format:"%h" -1`"'" .
echo build linux arm ...
CGO_ENABLED=0 GOOS=linux GOARCH=arm go build -o c-get-linux-arm -ldflags "-X main._version_='"`git describe --abbrev=0 --tags`"' -X main._commit_='"`git log --pretty=format:"%h" -1`"'" .
echo build mac ...
